exports = module.exports = ($scope, $element, $imageResizer, $log, $timeout) ->
  ...
  # $timeout ->
  #   imageContainer = $element[0].querySelector ".images"
  #   $scope.masonry = new Masonry imageContainer
  # , 10
  ...
  $scope.fileChange = (files=[]) ->
    ...
    for file in files
      ...
      do (file) -> $imageResizer.createThumbnail file,
        thumbnailHeight: 300
        thumbnailWidth: 300
  ...
  $scope.removeImage = ($index) ->
    ...
    switch image.status
      ...
      else                              image.status = "to-delete-from-server"
